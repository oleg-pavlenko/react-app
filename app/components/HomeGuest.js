...
function HomeGuest() {
  ...
  useEffect(() => {
    if (state.username.value) {
      const delay = setTimeout(() => dispatch({ type: 'usernameAfterDelay' }), 750);
  ...
  useEffect(() => {
    if (state.email.value) {
      const delay = setTimeout(() => dispatch({ type: 'emailAfterDelay' }), 750);
      return () => clearTimeout(delay);
    }
  }, [state.email.value]);
  ...
  useEffect(() => {
    if (state.password.value) {
      const delay = setTimeout(() => dispatch({ type: 'passwordAfterDelay' }), 750);
      return () => clearTimeout(delay);
    }
